...
interface DungeonButtonProps {
    title: string;
    dispatchDungeon: React.Dispatch<DungeonAction>;
    screenHeight: number
}

const DungeonButton: React.FC<DungeonButtonProps> = ({ title, dispatchDungeon, screenHeight }) => {
    const [pressDimensions, setPressDimensions] = useState<{ width: number, height: number }>({ width: 284, height: 108 });
    ...
    return (
        <Pressable key={title} style={styles.dungeon_button}
            onPressIn={() => dispatchDungeon({ dungeon: title, imageHeight: screenHeight })}
            onLayout={(e) => getDimensions(e)}
            accessibilityLabel={`Venture into ${title}`}
        >
            <Text style={[styles.dungeon_name_text, {
                fontSize: fontSize
            }]}>
                {title === "Undercity" ? `${title} (Initiative Only)` : title}
            </Text>
        </Pressable>
    )
}
        ...
        ]}>
            {
                currentDungeon.name === undefined ?
                    <>
                        ...
                        <View style={{
                            ...
                            {dungeonList.map((d: string) => {
                                return (
                                    <DungeonButton key={d} title={d} dispatchDungeon={dispatchDungeon} screenHeight={height} />
                                )
                            })
                            }
                        </View>
                    ...
                    <>
                        ...
                        >
                            <Svg viewBox='0 0 512 482' style={{ height: '100%', width: '100%' }}>
